...
#'@keywords internal
calculate_degrees_of_freedom = function(
  ...
  block_nesting = c(calculate_block_nesting(
    blockgroups,
    existingBlockStructure
  ))
  ...
  if (length(unique(block_nesting)) == length(block_nesting)) {
    ...
    stop(
      "All blocks are not uniquely nested: design has a random block structure. ",
  ...
  splitlayerlist = calculate_split_columns(
    run_matrix_processed,
    blockgroups,
    existingBlockStructure
  )
  ...
  if (!nointercept) {
    numberblocks = c(
      1,
      unlist(lapply(blockgroups, length)),
  ...
  interactions = is_intralayer_interaction(
    run_matrix_processed,
    model,
    splitlayer
  )
      ...
      if (split_plot_structure) {
        ...
        for (j in seq_len(length(interaction_cols_layer))) {
          if (interaction_cols_layer[j]) {
            ...
            higherorder_terms = unlist(grepl(
              "I\\((.+)\\^.+\\)",
              interaction_cols,
              perl = TRUE
            ))
            if (any(higherorder_terms)) {
              for (term in 1:length(higherorder_terms)) {
                if (higherorder_terms[term]) {
                  interaction_cols[term] = gsub(
                    "I\\(",
                    "",
                    interaction_cols[term],
                    perl = TRUE
                  )
                  interaction_cols[term] = gsub(
                    "\\^.+\\)",
  ...
  degrees_of_freedom = calc_interaction_degrees(
    run_matrix_processed,
    model,
    contrasts,
    nointercept,
