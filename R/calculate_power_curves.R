...
calculate_power_curves = function(trials,
  ...
  if(!(length(find.package("ggplot2", quiet = TRUE)) > 0) && plot_results) {
    warning("{ggplot2} package required for plotting results")
    plot_results = FALSE
  }
  if(plot_results) {
    if(grid_plot) {
      if(!length_2_effect) {
        print(ggplot2::ggplot(all_results) +
                ggplot2::geom_line(ggplot2::aes(x=trials, y = power, color=parameter)) +
                ggplot2::facet_grid(effectsize~type,
                           labeller = ggplot2::labeller(effectsize = ggplot2::label_both,
                                                        type = ggplot2::label_value)) +
                ggplot2::scale_y_continuous(limits=c(0,1), breaks = seq(0,1,by=0.2)) + ggplot_elements)
      } else {
        if(length(unique(all_results$effectsize_low)) == 1) {
          print(ggplot2::ggplot(all_results) +
                  ggplot2::geom_line(ggplot2::aes(x=trials, y = power, color=parameter)) +
                  ggplot2::facet_grid(effectsize_high~type,
          ...
          print(ggplot2::ggplot(all_results) +
                  ggplot2::geom_line(ggplot2::aes(x=trials, y = power, color=parameter)) +
                  ggplot2::facet_grid(effectsize_low~type,
                             labeller = ggplot2::labeller(effectsize_low = ggplot2::label_both, type = ggplot2::label_value)) +
                  ggplot2::scale_y_continuous(limits=c(0,1), breaks = seq(0,1,by=0.2)) + ggplot_elements)
        } else {
          print(ggplot2::ggplot(all_results) +
                  ggplot2::geom_line(ggplot2::aes(x=trials, y = power, color=parameter)) +
                  ggplot2::facet_grid(effectsize_low + effectsize_high~type,
      ...
      if(only_effect) {
        if(!length_2_effect) {
          ...
          if(length(unique(all_results$effectsize_low)) == 1) {
            ...
            print(ggplot2::ggplot(plot_results) +
                    ggplot2::geom_line(ggplot2::aes(x=effectsize_high, y = power, color=parameter)) +
                    ggplot2::facet_grid(effectsize_low~type,
