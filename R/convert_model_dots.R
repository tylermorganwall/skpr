...
convert_model_dots = function(RunMatrix, model) {
  if (any(unlist(strsplit(as.character(model[2]), "\\s\\+\\s|\\s\\*\\s|\\:|\\^")) == ".")) {
    ...
    powerterms = unlist(lapply(lapply(strsplit(additionterms, split = "\\^"), gsub, pattern = "^\\.$", replacement = dotreplace), paste0, collapse = "^"))

    multiplyterms = unlist(lapply(lapply(strsplit(powerterms, split = "\\s\\*\\s"), gsub, pattern = "^\\.$", replacement = dotreplace), paste0, collapse = " * "))
