#'@title Evaluates Power for Survival Design
#'
...
#'@param censor Function of the form f(Y,cutoff), where Y is the random vector generated by rfunction and
...
eval_design_survival_mc = function(RunMatrix, model, alpha, nsim, distribution, rfunction, censor, cutoff,
  ...
  if(!parallel) {
    ...
    for (j in 1:nsim) {
      ...
      RunMatrixReduced$Censor = censor(RunMatrixReduced$Y,cutoff)
      RunMatrixReduced$Y[RunMatrixReduced$Censor] = cutoff
      model_formula = update.formula(model, Surv(time=Y,event=!Censor) ~ .)
