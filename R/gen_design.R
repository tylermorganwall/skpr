...
#'
#'#Experimental coordinate exchange with original-unit numeric levels and constraints:
#'if(skpr:::run_documentation()) {
#'ce_candidates = expand.grid(temp = c(25, 50, 75),
#'                            pressure = c(1, 3, 5))
#'gen_design(ce_candidates, ~temp + pressure + temp:pressure, trials = 6,
...
gen_design = function(
  ...
  if (use_coordinate_exchange && !splitplot) {
    ...
    ce_factor_levels_original = ce_constraint_space$factor_levels
    ce_constraint_factor_levels = ce_factor_levels_original
    ...
    ce_factor_levels_override = advancedoptions[["factor_levels"]]
    ce_factor_levels_original_override =
      advancedoptions[["factor_levels_original"]]

    validate_ce_level_override(ce_factor_levels_override, "factor_levels")
    validate_ce_level_override(
      ce_factor_levels_original_override,
      "factor_levels_original"
    )
    factor_level_overlap = intersect(
      names(ce_factor_levels_override),
      names(ce_factor_levels_original_override)
    )
    if (length(factor_level_overlap) > 0) {
      stop(
        "skpr: Supply only one of advancedoptions$factor_levels or ",
        "advancedoptions$factor_levels_original for factor(s): ",
        paste(factor_level_overlap, collapse = ", ")
      )
    }
    ...
    if (!is.null(ce_factor_levels_original_override)) {
      for (nm in intersect(
        names(ce_factor_meta),
        names(ce_factor_levels_original_override)
      )) {
        override_levels = ce_factor_levels_original_override[[nm]]
        if (length(override_levels) == 0) {
          stop(
  ...
  if (use_coordinate_exchange && !splitplot) {
    ce_error_idx = which(vapply(
      genOutput,
      inherits,
      logical(1),
      what = "error"
    ))
    if (length(ce_error_idx) > 0) {
      first_ce_error = conditionMessage(genOutput[[ce_error_idx[[1]]]])
      if (length(ce_error_idx) == length(genOutput)) {
        stop(
          "skpr: Coordinate exchange failed in all ",
          length(genOutput),
          " repeats; first error: ",
          first_ce_error
        )
      ...
      warning(
        "skpr: Coordinate exchange failed in ",
        length(ce_error_idx),
        " of ",
        length(genOutput),
        " repeats; first error: ",
  ...
  for (i in seq_len(length(genOutput))) {
    if (
      is.null(genOutput[[i]]) ||
        inherits(genOutput[[i]], "error") ||
        is.null(genOutput[[i]][["criterion"]])
    ) {
      next
    }
