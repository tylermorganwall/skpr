...
#'
#'@param factorial A factorial test matrix generated for all combinations of the factors in the model
#'@param model The model used to generate the test design
#'@param trials The number of trials
#'@param optimality The optimality criterion (e.g. "D")
#'@param contrastslist A list of the contrasts (e.g. "contr.sum")
#'@param repeats The number of times to repeat design generation the for best optimal condition
#'@return The model matrix for the design, to be passed to eval_design
#'@export
...
gen_design = function(factorial, model, trials, optimality, contrastslist,repeats) {
  ...
  if(any(sapply(factorial,class) == "factor")) {
    ...
    dfmodelmatrix = optFederov(model,data=factorial,nTrials=trials,criterion = optimality,nRepeats = repeats)
