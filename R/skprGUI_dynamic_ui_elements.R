...
#' @return Shiny UI
#' @keywords internal
generate_factor_input_panel = function(
  factor_n = 1,
  factor_input_cache = NULL
) {
    ...
    single_panel = shiny::wellPanel(
      ...
      shiny::fluidRow(
        shiny::column(
          width = 5,
          shiny::selectInput(
            inputId = sprintf("blockdepth%i", factor_n),
        ...
        shiny::column(
          width = 7,
          shiny::selectInput(
            inputId = sprintf("factortype%i", factor_n),
            choices = list(
              "Continuous" = "numeric",
              "Categorical" = "cat",
              "Discrete Numeric" = "discnum"
            ),
            selected = default_val(
              factor_input_cache[[factortype_n]],
              "Continuous"
            ),
            label = "Type"
          )
      ...
      shiny::fluidRow(
        shiny::column(
          width = 12,
          shiny::textInput(
            inputId = sprintf("factorname%i", factor_n),
            value = default_val(
              factor_input_cache[[factorname_n]],
              sprintf("X%i", factor_n)
            ),
            label = "Name"
          )
        )
      ...
      shiny::conditionalPanel(
        ...
        shiny::fluidRow(
          shiny::column(
            width = 4,
            shiny::numericInput(
              inputId = sprintf("numericlow%i", factor_n),
          ...
          shiny::column(
            width = 4,
            shiny::numericInput(
              inputId = sprintf("numerichigh%i", factor_n),
          ...
          shiny::column(
            width = 4,
            shiny::numericInput(
              inputId = sprintf("numericlength%i", factor_n),
      ...
      shiny::conditionalPanel(
        ...
        shiny::fluidRow(
          shiny::column(
            width = 12,
            shiny::textInput(
              inputId = sprintf("disclevels%i", factor_n),
      ...
      shiny::conditionalPanel(
        ...
        shiny::fluidRow(
          shiny::column(
            width = 12,
            shiny::textInput(
              inputId = sprintf("levels%i", factor_n),
