...
Rcpp::List genOptimalDesignCoordinateExchangeConstrained(
  ...
  Eigen::MatrixXd V = (X.transpose() * X).partialPivLu().inverse();
  ...
  for (int iter = 0; iter < max_iter; ++iter) {
    ...
    int k = k_use - augmentedrows;
    if (k < 1)
      k = 1;

    for (int i = augmentedrows; i < n; ++i) {
      if (have_constraints && mustchange[i]) {
        pq.emplace(INFINITY, i);
        continue;
      }
      Eigen::VectorXd fi = X.row(i).transpose();
      double v_i = fi.dot(V * fi);
      double key = -v_i;
    ...
    for (int sel = 0; sel < k && !pq.empty(); ++sel) {
      Rcpp::checkUserInterrupt();
      int irow = pq.top().second;
      pq.pop();
    ...
    if (recompute_every > 0 && ((iter + 1) % recompute_every == 0)) {
      V = (X.transpose() * X).partialPivLu().inverse();
      logdet = logdet_xtx(X);
    }
