...
test_that("eval_design example code runs without errors", {
  ...
  expect_silent({
    coffeeblockdesign = gen_design(factorialcoffee, ~caffeine, trials = 12)
  })
  expect_silent({
    coffeefinaldesign = gen_design(
      factorialcoffee,
      model = ~ caffeine + cost + size + type,
      trials = 36,
...
test_that("gen_design parallel example code runs without errors", {
  ...
  expect_warning(
    eval_design_mc(temp, ~., 0.2, nsim = 10, glmfamily = "poisson"),
    " This can lead to unrealistic effect sizes"
  )
})
...
test_that("eval_design_mc example code runs without errors", {
  ...
  expect_silent(eval_design_mc(
    design = designcoffee,
    model = ~ cost + type + size + cost * type,
    0.05,
    nsim = 100,
    glmfamily = "gaussian"
  ))
  ...
  vhtcdesign = gen_design(
    candidateset = factorialcoffee,
    model = ~Store,
    trials = 8
  )
  htcdesign = gen_design(
    candidateset = factorialcoffee,
    model = ~ Store + Temp,
    trials = 24,
    splitplotdesign = vhtcdesign,
    blocksizes = 3
  )
  expect_silent({
    splitplotdesign = gen_design(
      candidateset = factorialcoffee,
      model = ~ Store + Temp + cost + type + size,
      trials = 96,
      splitplotdesign = htcdesign,
      blocksizes = 4,
      varianceratio = 3
    )
  })
  ...
  expect_error(
    eval_design_mc(
      ...
      nsim = 1,
      glmfamily = "gaussian",
      varianceratios = c(5, 4, 2, 2)
    ),
    "Wrong number of variance ratios specified."
  )
  expect_silent(eval_design_mc(
    design = splitplotdesign,
    model = ~ Store + Temp + cost + type + size,
  ...
  expect_silent(eval_design_mc(
    ...
    anticoef = c(1.5, 0.7, 0.7),
    glmfamily = "binomial",
    advancedoptions = list(anovatest = "LR")
  ))
  expect_silent(eval_design_mc(
    designbinom,
    ~ a + b,
  ...
  expect_silent(eval_design_mc(
    ...
    anticoef = c(1.5, 0.7, 0.7),
    glmfamily = "binomial",
    advancedoptions = list(anovatest = "F")
  ))
  expect_silent(eval_design_mc(
    designbinom,
    ~ a + b,
  ...
  expect_silent(eval_design_mc(
    ...
    anticoef = c(1.5, 0.7, 0.7),
    glmfamily = "gaussian",
    advancedoptions = list(anovatype = "II")
  ))
  expect_silent(eval_design_mc(
    designbinom,
    ~ a + b,
  ...
  expect_silent(eval_design_mc(
    ...
    anticoef = c(1.5, 0.7, 0.7),
    glmfamily = "binomial",
    advancedoptions = list(anovatest = "LR", anovatype = "II")
  ))
  expect_silent(eval_design_mc(
    designbinom,
    ~ a + b,
  ...
  expect_silent(eval_design_mc(
    ...
    anticoef = c(1.5, 0.7, 0.7),
    glmfamily = "binomial",
    advancedoptions = list(anovatest = "F", anovatype = "II")
  ))
  expect_silent(eval_design_mc(
    designbinom,
    ~ a + b,
...
test_that("eval_design_survival_mc example code runs without errors", {
  ...
  expect_warning(
    {
      a = eval_design_survival_mc(
        design = design,
        model = ~a,
        alpha = 0.05,
        nsim = 100,
        distribution = "exponential",
        rfunctionsurv = rsurvival,
        effectsize = 1
      )
    },
  ...
  expect_silent(
    eval_design_survival_mc(
      ...
      rfunctionsurv = rlognorm,
      anticoef = c(0.184, 0.101),
      scale = 0.4
    )
  )
  #testing parallel
...
test_that("eval_design_custom_mc example code runs without errors", {
  ...
  #'
  expect_silent(eval_design_custom_mc(
    design = design,
    model = ~a,
    alpha = 0.05,
    nsim = 100,
    fitfunction = fitsurv,
    pvalfunction = pvalsurv,
    rfunction = rsurvival,
    effectsize = 1
  ))
...
test_that("eval_design_survival_mc does not error with categoricals related to pvalues", {
  candidateset = expand.grid(
    X1 = seq(-1, 1, length.out = 3),
    X2 = c("a", "b", "c"),
    X3 = seq(-1, 1, length.out = 3),
    X4 = seq(-1, 1, length.out = 3),
    X5 = seq(-1, 1, length.out = 3)
  )

  # Generating design:
  design = gen_design(
    candidateset = candidateset,
    model = ~ (X1 + X2 + X3 + X4 + X5) + I(X1^2) + X1:X2,
    trials = 12
  )

  # Evaluating (Monte Carlo Survival) Design:
  expect_silent({
    eval_design_survival_mc(
      design = design,
      model = ~ . + I(X1^2) + X1:X2,
